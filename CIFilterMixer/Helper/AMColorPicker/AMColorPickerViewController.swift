...
public class AMColorPickerViewController: UIViewController {
    ...
    @IBAction private func tappedCloseButton(_ sender: UIButton) {
        dismiss(animated: true) {
            self.delegate?.finalColor(color: self.selectedColor, selectedFilter: self.selectedFilter)
        }
    }
}

extension AMColorPickerViewController: AMColorPickerViewDelegate {
    public func colorPicker(_ colorPicker: UIView, didSelect color: UIColor) {
